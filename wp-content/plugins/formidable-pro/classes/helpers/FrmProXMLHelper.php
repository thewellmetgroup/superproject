	...
	*/
	...
	private static function set_values_for_fields( $key, $field_id, $data, &$values ) {
		...
		if ( $field->field_options['in_section'] ) {
			...
			if ( isset( $importing_fields[ $section_id ] ) ) {
				$section = $importing_fields[ $section_id ];
			} else {
				$section = FrmField::getOne( $section_id );

				if ( $section ) {
					$importing_fields[ $section_id ] = $section;
				}
			}
		...
		} elseif ( isset( $_POST['form_id'] ) && $field->form_id !== $_POST['form_id'] ) {
			$is_embedded_field = true;
			$fields            = FrmField::get_all_for_form( $_POST['form_id'], '', 'exclude', 'exclude' );
			foreach ( $fields as $parent_form_field ) {
				if ( (int) $parent_form_field->field_options['form_select'] === (int) $field->form_id ) {
					$section_id = $parent_form_field->id;
					break;
				}
			}
			unset( $fields );
		...
		if ( $field->type === 'user_id' ) {
			$_POST['frm_user_id'] = $values['frm_user_id'] = $value;
		}

		if ( $is_repeater_child || $is_embedded_field ) {
