...
class FrmProFileField {
	...
	*/
	...
	public static function maybe_set_chmod( $args ) {
		$single_file = isset( $args['file_id'] );

		if ( isset( $args['form_id'] ) ) {
			$form_id = $args['form_id'];
		} elseif ( ! $single_file ) {
			// if no file id or form id is passed, do nothing
			return;
		}

		if ( $single_file && empty( $form_id ) ) {
			$form_id = self::get_form_id_from_file_id( $args['file_id'] );
		}

		$protected = isset( $args['protected'] ) ? $args['protected'] : self::folder_is_protected( $form_id );

		if ( $single_file ) {
			self::set_file_protection( get_attached_file( $args['file_id'] ), $protected );
			return;
		}
		...
		if ( file_exists( $dir ) ) {
			...
			foreach ( $scan as $value ) {
				if ( in_array( $value, array( '.', '..', '.htaccess' ), true ) ) {
					continue;
				}

				$file = "$dir/$value";
				if ( is_file( $file ) ) {
					self::set_file_protection( $file, $protected );
				}
			}
		}

		$child_form_ids = FrmProForm::get_child_form_ids( $form_id );
		foreach ( $child_form_ids as $child_form_id ) {
			self::maybe_set_chmod( array( 'form_id' => $child_form_id, 'protected' => $protected ) );
		}
	}
	...
	public static function get_file_url( $id, $size = false, $args = array() ) {
		...
		if ( isset( $args['url'] ) ) {
			...
			if ( $form_id !== -1 ) {
				$url = self::upload_dir_url( $form_id ) . '/' . basename( get_attached_file( $id ) );
			} else {
		...
		if ( ! $protected ) {
			...
			if ( isset( $args['src'] ) ) {
				...
				$url = $size === 'thumbnail' ? wp_get_attachment_thumb_url( $id ) : '';
			}
		...
		$filename = str_replace( self::upload_dir_url( $form_id ) . '/', '', $url );
	...
	public static function check_for_download() {
		...
		if ( 200 === $download['code'] ) {
			chmod( $download['path'], 0400 );

			$mime_type   = mime_content_type( $download['path'] );
	...
	private static function should_filter_url( $url, $attachment_id ) {
		...
		return self::url_matches_upload_dir_path( $url, $form_id );
	}
	 ...
	 * @return bool
	 */
	private static function url_matches_upload_dir_path( $url, $form_id ) {
		$expected_base_url = self::upload_dir_url( $form_id );
		return strpos( $url, $expected_base_url ) !== false;
	}
