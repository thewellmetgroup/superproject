...
class FrmProForm {
	...
	private static function setup_file_protection( $atts ) {
		...
		if ( $previous_val == $atts['new'] ) {
			// do nothing if the value has not actually changed
			return;
		}

		$folder_name = FrmProFileField::get_upload_dir_for_form( $atts['form_id'] );
		if ( ! $folder_name ) {
			// do nothing if we do not get back a valid folder name
			return;
		}

		self::maybe_update_htaccess_file( $atts['form_id'], $atts['new'] );
		FrmProFileField::maybe_set_chmod( array( 'form_id' => $atts['form_id'], 'protected' => $atts['new'] ) );
	}
	 ...
	 * @param bool $deny
	 */
	private static function maybe_update_htaccess_file( $form_id, $deny ) {
		if ( ! FrmProFileField::server_supports_htaccess() ) {
			return;
		}

		$folder_name = FrmProFileField::get_upload_dir_for_form( $form_id );
		...
		foreach ( $child_form_ids as $child_form_id ) {
			self::maybe_update_htaccess_file( $child_form_id, $deny );
		}
	}
	...
	public static function maybe_fix_conditions( $id, $map ) {
		...
		$map = array_reduce( $field_ids, function( $total, $field_id ) use ( $map ) {
			$current = $map[ $field_id ];
			$current = array_filter( $current, function( $value, $key ) {
				return $value !== $key;
